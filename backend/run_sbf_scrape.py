"""
Gorev 3.2.8.3 — Saglik Bilimleri Fakultesi: Harita Gudumlu Deep Scrape

Fakulte genel (BirimID=21) + Ebelik + Hemsirelik + Fizyoterapi
"""
import sys
import json
...
SBF_DIR = GIBTU / "FAKÜLTELER" / "Sağlık Bilimleri Fakültesi"

BLUEPRINTS = [
    {
        "file": SBF_DIR / "Sağlık_Bilimleri_Fakültesi.html",
        "department": "Saglik Bilimleri Fakultesi",
        "contact_unit": "SBF Dekanligi",
        "contact_info": "sbf@gibtu.edu.tr",
    },
    {
        "file": SBF_DIR / "Ebelik.html",
        "department": "Ebelik Bolumu",
        "contact_unit": "Ebelik Bolumu",
        "contact_info": "ebelik@gibtu.edu.tr",
    },
    {
        "file": SBF_DIR / "Hemşirelik.html",
        "department": "Hemsirelik Bolumu",
        "contact_unit": "Hemsirelik Bolumu",
        "contact_info": "hemsirelik@gibtu.edu.tr",
    },
    {
        "file": SBF_DIR / "Fizyoterapi_ve_Rehabilitasyon_Bölümü.html",
        "department": "Fizyoterapi ve Rehabilitasyon Bolumu",
        "contact_unit": "Fizyoterapi ve Rehabilitasyon Bolumu",
        "contact_info": "fizyoterapi@gibtu.edu.tr",
    },
]
...
def phase1_blueprint_check():
    print("=" * 65)
    print("FAZ 1: Blueprint Parse — SBF Tum Birimler")
    print("=" * 65)
    total_urls = 0
...
def phase2_full_scrape():
    print("\n" + "=" * 65)
    print("FAZ 2: Canli Deep Scrape — SBF Tum Birimler")
    print("=" * 65)
    all_reports = []
    for bp_info in BLUEPRINTS:
        bp_file = bp_info["file"]
        ...
        json_path = OUTPUT_DIR / f"sbf_{safe_name}_output.json"
...
def phase3_validation(all_reports):
    print("\n" + "=" * 65)
    print("FAZ 3: Dogrulama — SBF Toplam")
    print("=" * 65)
    checks = []
    ...
    print(f"\n  TOPLAM: {total_docs} doc, {total_valid} valid, "
          f"{total_failed} failed, {total_chars:,} chars")
    ...
    for e in all_reports:
        check(f"{e['department'][:30]} docs>=5", e["report"].total_documents >= 5,
...
def main():
    ...
    print("\n" + "=" * 65)
    print(f"GOREV 3.2.8.3 — SBF Deep Scrape {'TAMAMLANDI' if success else 'BASARISIZ'}!")
    print("=" * 65)

    summary = {"task": "3.2.8.3", "description": "SBF Harita Gudumlu Deep Scrape", "birimler": []}
    for entry in all_reports:
        r = entry["report"]
    ...
    summary_path = OUTPUT_DIR / "sbf_summary.json"
